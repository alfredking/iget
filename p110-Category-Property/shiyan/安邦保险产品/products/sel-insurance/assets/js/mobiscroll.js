    ...
    function Scroller(elem, settings) {
        ...
        function read() {
            if(elm.val()){
                ...
                that.temp = ((input && that.val !== null && that.val != elm.text()) || that.values === null) ? s.parseValue(elm.text() || '', that) : that.values.slice(0);
            }
            that.setValue(true);
        }
        ...
        */
        that.show = function (prevAnim) {
            ...
            if(s.isCertificate){
                html += (s.display != 'inline' ? '<div class="dwbc' + (s.button3 ? ' dwbc-p' : '') + '"><span class="dwbw dwb-l" style=" width:100%; border-bottom: 1px solid #b6d2ff;"><span class="dwb" style="color: #4a8dfa;background-color: #eaf2ff;">' + s.longeText + '</span></span><span class="dwbw dwb-s"><span class="dwb">' + s.setText + '</span></span>' + (s.button3 ? '<span class="dwbw dwb-n"><span class="dwb">' + s.button3Text + '</span></span>' : '') + '<span class="dwbw dwb-c"><span class="dwb">' + s.cancelText + '</span></span></div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
            }else{
            ...
            if (s.display != 'inline') {
                ...
                that.tap($('.dwb-l span', dw), function () {
                    if (that.hide(false, 'longe') !== false) {
                        $(e).text(s.longeText).trigger('change');
                    }
                });
        ...
        that.init = function (ss) {
            ...
            if (s.display == 'inline') {
                ...
                if (input && s.showOnFocus) {
                    ...
                    e.readOnly = true;
                    // Init show datewheel
                    elm.bind('click', function (e) { that.show(); e.stopPropagation();} );
                }
            }
        };
