...
- (void)stopGIF{
    [self.renderQueue addOperationWithBlock:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self->_gifSourceRef) {
                CFRelease(self->_gifSourceRef);
                self->_gifSourceRef = nil;
            }
...
- (void)play{
    ...
    if (_timestamp >= _currentProgress) {
        ...
        if(self.renderQueue.operationCount<=0) {
            [self.renderQueue addOperationWithBlock:^{
                CGImageRef ref = CGImageSourceCreateImageAtIndex(self->_gifSourceRef, _index%_frameCount, NULL);
                dispatch_async(dispatch_get_main_queue(), ^{
