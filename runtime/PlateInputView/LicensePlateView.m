...
+ (instancetype)initFrame:(CGRect)frame OriginalString:(NSString *)originalString block:(void (^)(NSString *))block {
    ...
    carNumberView.totalTextStr = originalString;
...
- (void)provinceClick:(UIButton *)sender{
    ...
    if ((_totalTextStr.length == 0 || _totalTextStr.length == 8) && [sender.titleLabel.text isEqualToString:@"ABC"]) {
        return;
    }
    ...
    if (index == 30) {//    切换英文输入
        ...
        NSString *textStr = [NSString stringWithFormat:@"%@%@",_totalTextStr,_provinceStr];
...
- (void)letterClick:(UIButton *)sender{
    ...
    if (_totalTextStr.length > 0 && [sender.titleLabel.text isEqualToString:@"返回"]) {
        return;
    }
...
//        self.letterStr = [NSString stringWithFormat:@"%@%@",_letterStr,_letterArr[index]];
        NSString *textStr = [NSString stringWithFormat:@"%@%@",_totalTextStr,_letterArr[index]];
        if (self.sendTextBlock) {
            self.sendTextBlock(textStr);
            _totalTextStr = textStr;
        }
...
- (void)deleteText{
    ...
    if (_totalTextStr.length > 0) {
        ...
        self.letterStr = @"";
        self.totalTextStr = @"";
//        self.sendTextBlock(self.totalTextStr);
    }
}
