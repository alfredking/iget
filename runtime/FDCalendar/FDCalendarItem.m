...
- (void)setDateArray:(NSArray<NSDate *> *)dateArray{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if (self.dateArray.count > 0) {
            ...
            if ([[NSCalendar currentCalendar] component:NSCalendarUnitMonth fromDate:self.date] == [[NSCalendar currentCalendar] component:NSCalendarUnitMonth fromDate:[NSDate date]]) {
                
                for (NSInteger i = 0; i < self.dateArray.count; i ++) {
                    NSInteger arrDay = [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.dateArray[i]];
                    
                    if ([[NSCalendar currentCalendar] component:NSCalendarUnitMonth fromDate:self.date] == [[NSCalendar currentCalendar] component:NSCalendarUnitMonth fromDate:self.dateArray[i]]) {
                        
                        if (day == arrDay) {
                            cell.backgroundColor = [UIColor colorWithHexString:NavBarColorHex];
                            cell.layer.cornerRadius = cell.frame.size.height / 2;
                            cell.dayLabel.textColor = [UIColor whiteColor];
                            cell.chineseDayLabel.textColor = [UIColor whiteColor];
                        }
                        
                        if ([[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date] == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.dateArray[i]]) {
                            if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.dateArray[i]]) {
...
//                            NSDate *tampDate = [formatter dateFromString:str];
//                            
//                            NSTimeZone *zone = [NSTimeZone systemTimeZone];
//                            NSInteger interval = [zone secondsFromGMTForDate:tampDate];
//                            NSDate *localDate = [tampDate dateByAddingTimeInterval:interval];
                            
                            
                            if ([self.dateArray containsObject:self.date]) {
            ...
            if ([[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth] && ![[NSCalendar currentCalendar] isDateInToday:self.date]) {
                ...
                if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:[NSDate date]]) {
                    cell.dayLabel.textColor = [UIColor colorWithHexString:NavBarColorHex];
                }
            }
