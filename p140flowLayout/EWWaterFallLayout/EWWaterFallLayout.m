...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"***************** layoutAttributesForItemAtIndexPath indexPath is %ld",(long)indexPath);
    for (int i = 0; i < self.columnCount; i++) {
        //取得第i列的高度
        CGFloat columnHeight = [self.columnHeights[i] doubleValue];
        
        NSLog(@"in the loop minColumnHeight is %ld",(long)minColumnHeight);
        NSLog(@"in the loop columnHeight is %ld",(long)columnHeight);
        ...
        if (minColumnHeight > columnHeight) {
            ...
            NSLog(@"in the loop destColumn is %ld",(long)destColumn);
        }
    }
    
    NSLog(@"final destColumn is %ld",(long)destColumn);
    ...
    //更新最短那一列的高度,CGRectGetMaxY表示把cell高度加上了
