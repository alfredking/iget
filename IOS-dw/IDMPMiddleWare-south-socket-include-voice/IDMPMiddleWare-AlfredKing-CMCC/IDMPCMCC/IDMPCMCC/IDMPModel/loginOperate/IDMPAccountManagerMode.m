...
- (void)registerUserWithPhoneNo:(NSString *)phoneNo passWord:(NSString *)password andValidCode:(NSString *)validCode traceId:(NSString *)traceId finishBlock:(accessBlock)successBlock failBlock:(accessBlock )failBlock
{
    NSDictionary *result;
    if (phoneNo.length == 0)
    {
...
//    }
    ...
    NSString *UserManage = [NSString stringWithFormat:@"%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\"",Rgclientversion,version,sdkversion,sdkversionValue,TMIsdn,phoneNo,ksEnPasswd,encryptPassWd,Validcode,validCode,ksEnClientNonce,encryptClientNonce,ksIOS_ID,[IDMPDevice getDeviceID],IDMPTraceId,traceId];
...
//         NSLog(@"-------%@",request.responseStr);
    ...
    failBlock:
     ^(NSDictionary *parameters){
         NSLog(@" request %@ parameters is %@",vcUrl, parameters);
...
- (void)resetPasswordWithPhoneNo:(NSString *)phoneNo passWord:(NSString *)password andValidCode:(NSString *)validCode traceId:(NSString *)traceId finishBlock:(accessBlock)successBlock failBlock:(accessBlock)failBlock
{
    NSDictionary *result;
    if (phoneNo.length == 0)
    {
...
//    }
    ...
    NSString *encryptClientNonce= secRSA_Encrypt(clientNonce);
    NSString *encryptPassWd= secRSA_Encrypt(password);
    NSString *UserManage = [NSString stringWithFormat:@"%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\"",Rpclientversion,version,sdkversion,sdkversionValue,TMIsdn,phoneNo,ksEnPasswd,encryptPassWd,Validcode,validCode,ksEnClientNonce,encryptClientNonce,ksIOS_ID,[IDMPDevice getDeviceID],IDMPTraceId,traceId];
    NSString *Signature = secRSA_EVP_Sign(UserManage);
    NSDictionary *heads = [NSDictionary dictionaryWithObjectsAndKeys:UserManage,TMUserManage,Signature,ksSignature, nil];
    NSLog(@"请求头部%@",heads);
    IDMPHttpRequest *__block request = [[IDMPHttpRequest alloc]init];
...
- (void)changePasswordWithPhoneNo:(NSString *)phoneNo passWord:(NSString *)password andNewPSW:(NSString *)newpassword traceId:(NSString *)traceId finishBlock:(accessBlock)successBlock failBlock:(accessBlock)failBlock
{
    NSDictionary *result;
    if (phoneNo.length == 0)
    {
    ...
    NSString *UserManage = [NSString stringWithFormat:@"%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\"",Cpclientversion,version,sdkversion,sdkversionValue,TMIsdn,phoneNo,Encoldpasswd,encryptPassWd,ksEnPasswd,encryptNewPassWd,ksEnClientNonce,encryptClientNonce,ksIOS_ID,[IDMPDevice getDeviceID],IDMPTraceId,traceId];
    NSString *Signature = secRSA_EVP_Sign(UserManage);
    NSDictionary *heads = [NSDictionary dictionaryWithObjectsAndKeys:UserManage,TMUserManage,Signature,ksSignature, nil];
    NSLog(@"请求头部%@",heads);
    IDMPHttpRequest *__block request = [[IDMPHttpRequest alloc]init];
