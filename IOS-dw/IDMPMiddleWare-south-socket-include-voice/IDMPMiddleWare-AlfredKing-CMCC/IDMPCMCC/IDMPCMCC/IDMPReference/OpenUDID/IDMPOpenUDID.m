...
static NSString * kIDMPOpenUDIDSessionCache = nil;
//static NSString * const kOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const kIDMPOpenUDIDKey = @"OpenUDID";
static NSString * const kIDMPOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const kIDMPOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const kIDMPOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kIDMPOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kIDMPOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kIDMPOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kIDMPOpenUDIDRedundancySlots = 100;

@interface IDMPOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kIDMPOpenUDIDDomain];
#else
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:kIDMPOpenUDIDDomain];
#else
    ...
    if (item) {
        @try{
            ...
            IDMPOpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
//
+ (NSString*) value {
    return [IDMPOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {

    if (kIDMPOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kIDMPOpenUDIDDomain
                                         code:kIDMPOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
        return kIDMPOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:kIDMPOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:kIDMPOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:kIDMPOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:kIDMPOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:kIDMPOpenUDIDOOTSKey];
        optedOut = optedOutDate!=nil;
        IDMPOpenUDIDLog(@"localDict = %@",localDict);
    }
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kIDMPOpenUDIDRedundancySlots];
    for (int n=0; n<kIDMPOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kIDMPOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        UIPasteboard* slotPB = [UIPasteboard pasteboardWithName:slotPBid create:NO];
#else
        NSPasteboard* slotPB = [NSPasteboard pasteboardWithName:slotPBid];
#endif
        IDMPOpenUDIDLog(@"SlotPB name = %@",slotPBid);
        if (slotPB==nil) {
            // assign availableSlotPBid to be the first one available
            if (availableSlotPBid==nil) availableSlotPBid = slotPBid;
        } else {
            NSDictionary* dict = [IDMPOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:kIDMPOpenUDIDKey];
            IDMPOpenUDIDLog(@"SlotPB dict = %@",dict);
            if (oudid==nil) {
            ...
            NSString* gid = [dict objectForKey:kIDMPOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:kIDMPOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;   
                }
            }
        }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [IDMPOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) { 
            ...
            [localDict setObject:openUDID forKey:kIDMPOpenUDIDKey];
            [localDict setObject:appUID forKey:kIDMPOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:kIDMPOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:kIDMPOpenUDIDTSKey];
    ...
    //
    IDMPOpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:kIDMPOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [IDMPOpenUDID _setDict:localDict forPasteboard:slotPB];
    }

    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:kIDMPOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kIDMPOpenUDIDDomain
                                                     code:kIDMPOpenUDIDErrorOptedOut
        ...
        kIDMPOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return kIDMPOpenUDIDSessionCache;
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kIDMPOpenUDIDDomain
                                         code:kIDMPOpenUDIDErrorCompromised
    ...
    kIDMPOpenUDIDSessionCache = [openUDID retain];
    return kIDMPOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:kIDMPOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:kIDMPOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:kIDMPOpenUDIDOOTSKey];

  	// store the dictionary locally
    [defaults setObject:dict forKey:kIDMPOpenUDIDKey];
    
    IDMPOpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
    kIDMPOpenUDIDSessionCache = nil;
