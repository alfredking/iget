...
+(void)checkToken:(NSString *)Token andAppid:(NSString *)Appid andHmac:(NSString *)hmac
{
    NSString *sourceId = [[NSUserDefaults standardUserDefaults]objectForKey:source];
    NSDictionary *heads = [NSDictionary dictionaryWithObjectsAndKeys:Token,@"token",sourceId,@"sourceId",hmac,@"hmac", nil];
    NSDictionary *jsonDict = [IDMPHttpRequest dictToJSON:heads];
    IDMPHttpRequest *__block request = [[IDMPHttpRequest alloc]init];
    [request getHttpWithDict:jsonDict url:tokenValidateUrl successBlock:
        ^{
            NSLog(@"token request:%@",request.responseStr);
            
        }
        failBlock:
        ^{
            NSLog(@"network fail,%@",request.responseStr);
        }];
}


+ (void)checkToken:(NSString *)Token andAppid:(NSString *)Appid successBlock:(callBackBlock)success failBlock:(callBackBlock)fail
{
    NSString *sourceId = [[NSUserDefaults standardUserDefaults]objectForKey:source];
    NSDictionary *heads = [NSDictionary dictionaryWithObjectsAndKeys:Token,@"token",sourceId,@"sourceId", nil];
    NSDictionary *jsonDict = [IDMPHttpRequest dictToJSON:heads];
    NSLog(@"jsonDict:%@",jsonDict);
    IDMPHttpRequest *__block request = [[IDMPHttpRequest alloc]init];
    [request getHttpWithDict:jsonDict url:tokenUrl successBlock:
     ^{
         NSLog(@"token request:%@",request.responseStr);
         
         if (success) {
             success([request.responseStr objectForKey:@"body"]);
         }
     }
                   failBlock:
     ^{
