...
+ (void)renewKsWithAppid:(NSString *)appid appkey:(NSString *)appkey certID:(NSString *)certID mobile:(NSString *)mobile callBack:(UMCSDKCallBack)callBack
{
    ...
    [request getHttpByHeads:heads url:createKSUrl successBlock:^{
        ...
        NSLog(@"IDMPKsModel cnonce:%@",cnonce);
        ...
        if ([self checkKSIsValid:responseHeaders clientNonce:cnonce])
        {
            ...
            if(sourceid)
            {
                ...
                if ([sip isEqualToString:@"1"])
                {
                    ...
                    result=[NSDictionary dictionaryWithObjectsAndKeys:userName,@"mobileNumber",Token,@"token",authType,@"authType",@"102000",@"resultCode",passid,@"passid",expiretime,@"expiretime",nil];
                }
                else
                {
                    result=[NSDictionary dictionaryWithObjectsAndKeys:userName,@"mobileNumber",passid,@"passid",authType,@"authType",@"102000",@"resultCode",Token,@"token",expiretime,@"expiretime",nil];
                }
            }
            else
            {
