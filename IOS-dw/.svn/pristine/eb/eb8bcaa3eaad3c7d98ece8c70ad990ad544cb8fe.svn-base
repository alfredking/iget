...
//    printf("@ kdf_pw 1   %s    @2   %s   @3   %s   @4    %s \n",key,pscene,pnonce,pcnonce);
    ...
    NSLog(@"kdf_pw ks");
    ks = hmacWithSha256((char *)key,key_len,(char *)s,s_len);
    free(s);
    s=NULL;
    return ks;
}
...
+(NSString*)getNativeMac:(unsigned char*)ks data:(NSString *)wwwauthenticate
{
    ...
    mac = hmacWithSha256((char *)ks,16,(char *)[wwwauthenticate UTF8String],data_len);
    
    NSString *nativeMac=[NSString idmp_hexStringWithChar:mac length:32];
    free(mac);
    mac=NULL;
