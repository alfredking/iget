...
-(void)getDataSmsKSWithSuccessBlock:(accessBlock)successBlock failBlock:(accessBlock)failBlock;
{
    ...
    NSLog(@"new rand %@",Rand);
    NSArray *receiveList=[[NSArray alloc] initWithObjects:UpSmsNum,nil];
    NSString *sip=[[NSUserDefaults standardUserDefaults] objectForKey:getType];
    NSString *authorization = [NSString stringWithFormat:@"%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\"",ksDS_clientversion,version,SIP,sip, ksEnClientkek,encryptClientNonce,ksRAND,clientNonce,ksIOS_ID,[IDMPDevice getDeviceID]];
