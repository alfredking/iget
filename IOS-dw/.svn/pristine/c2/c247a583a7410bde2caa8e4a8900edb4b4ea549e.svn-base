...
+(NSDictionary *)getUserInfoWithError: (NSError **)fileError
{
    ...
    if (*fileError)
    {
        NSLog(@"first read error %d",(*fileError).code);
        userData = [NSData dataWithContentsOfFile:plistPath options:NSDataReadingMappedIfSafe  error:fileError];
        if (*fileError)
        {
            NSLog(@"second read error %d",(*fileError).code);
            userData = [NSData dataWithContentsOfFile:plistPath options:NSDataReadingMappedIfSafe  error:fileError];
            NSLog(@"third read error %d",(*fileError).code);
        }
    }
    ...
    if (!*fileError)
    {
        ...
        else
        {
            ...
            {
                NSLog(@"decrypt file data is exsit");
    ...
    else
    {
        ...
        NSLog(@"file read error:%d", (*fileError).code);
