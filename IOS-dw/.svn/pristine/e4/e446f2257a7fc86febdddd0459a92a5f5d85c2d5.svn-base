...
NSString *secRSA_Encrypt(NSString *sourceString)
{
    ...
    [locken lock];
    NSString *pubKey=cmccKomectSecPUBLIC_KEY;
    ...
    {
        [locken unlock];
        return backupRSA_encrypt(sourceString);
    }
    ...
    {
        ret =[[IDMPFormatTransform charToNSHex:outbuf length:(int)outlen] lowercaseString];
    }
    free(outbuf);
...
NSString *secRSA_Decrypt(NSString *sourceString)
{
    ...
    NSString *privKey=cmccKomectSecPRIVATE_KEY;
    ...
    if(!keyRef)
    {
        ...
        [lockde unlock];
        return backupRSA_decrypt(sourceString);;
    }
...
+ (SecKeyRef)addPublicKey:(NSString *)key
{
    ...
    if (!secPublicKeyRef)
    {
        ...
        NSString *tag = @"cmccKomectSecRSAUtil_PubKey";
...
+ (SecKeyRef)addPrivateKey:(NSString *)key
{
    ...
    if (!secPrivateKeyRef)
    {
        NSString *tag = @"cmccKomectSecRSAUtil_PrivKey";
        NSData *d_tag = [NSData dataWithBytes:[tag UTF8String] length:[tag length]];
...
NSString *backupRSA_encrypt(NSString *data)
{
    static NSLock *locken = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
    ...
    RSA *p_rsa;
    int rsa_len;
    NSString *public = formatPublicKey(cmccKomectSecPUBLIC_KEY);
...
NSString *backupRSA_decrypt(NSString *data)
{
    static NSLock *lockde = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
    ...
    [lockde lock];
    unsigned char *str = [IDMPFormatTransform hexStringToChar:data];
    unsigned char *p_de;
    RSA *p_rsa;
    int rsa_len;
    NSString *private = formatPrivateKey(cmccKomectSecPRIVATE_KEY);
