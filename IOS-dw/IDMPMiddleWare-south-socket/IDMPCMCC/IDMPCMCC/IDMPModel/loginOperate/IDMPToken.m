...
+ (NSString *)getTokenWithUserName:(NSString *)userName andSourceId:(NSString *)sourceid isTmpCache:(BOOL)isTmpCache
{
...
#endif
    unsigned char *token;
    int tokenLen =signkey((char *)[userName UTF8String],(char *)[sourceid UTF8String], version,isTmpCache, &token);
    if(tokenLen <= 0)
    {
        token=NULL;
        [lock unlock];
...
//    }
#ifdef STATESECRET
    NSString *stoken=[NSString idmp_hexStringWithChar:token length:tokenLen];
    free(token);
    token=NULL;
    [lock unlock];
    return stoken;
#else
    NSString *stoken=[NSString idmp_hexStringWithChar:token length:tokenLen];
    NSString *mac = [[stoken substringFromIndex:stoken.length - 64] uppercaseString];
    NSString *encMac = [mac idmp_getMd5String];
    NSString *finalToken = [stoken stringByReplacingCharactersInRange:NSMakeRange(stoken.length - 64, 64) withString:encMac];
    free(token);
    token=NULL;
    [lock unlock];
    return finalToken;
...
int signkey(char *userName,char *appid, char* version, bool isTmpCache, unsigned char** ptoken)
{
    if (!isTmpCache) {
        ...
        NSString *SQN=[user objectForKey:IDMPSQN];
        if(!BTID||!KS||!SQN)
        {
            return 0;
        }
        NSUInteger ISQN=[SQN integerValue]+1;
        [user setValue:[NSString stringWithFormat:@"%d",(int)ISQN] forKey:IDMPSQN];
        ...
        NSString *BTID=[tmpUser objectForKey:IDMPBTID];
        g_btid = (unsigned char *)[BTID UTF8String];
        g_btid_len=(int)[BTID length];
        NSString *KS=[tmpUser objectForKey:IDMPKS];
        unsigned char *g_ks_temp = (unsigned char *)[KS UTF8String];
        g_ks= HexStrToByte((const char*)g_ks_temp,32);
        NSString *SQN=[tmpUser objectForKey:IDMPSQN];
        if(!BTID||!KS||!SQN)
        {
            return 0;
        }
...
//    printf("@kdf_signkey 1   %s    @2   %s   @3   %s   @4    %s \n",pscene,rand,impi,appid);
    ...
    psk = hmacWithSha256((char *)g_ks,g_ks_len,(char *)s,s_len);
    free(g_ks);
    g_ks=NULL;
    free(s);
    s=NULL;
...
int sign_token(char* str_name, char  *version,char* str_sourceid, unsigned char** ptoken, unsigned char* psk, int psk_len)
{
    ...
    unsigned char* token = malloc(2 + token_head_len + mac_len + 3);
    ...
    NSLog(@"caculate mac");
#ifdef STATESECRET
    NSString *pskStr = [[NSString idmp_hexStringWithChar:psk length:psk_len] uppercaseString];
    NSString *tokenStr = [NSString idmp_hexStringWithChar:(unsigned char *)(token+2) length:token_head_len];
    NSString *pskToken = [NSString stringWithFormat:@"%@%@",pskStr,tokenStr];
    NSString *macStr = [IDMPUtility sm3WithRawData:pskToken];
    mac = HexStrToByte([macStr UTF8String], mac_len * 2);
#else
    mac = hmacWithSha256((char *)psk, psk_len,(char *)(token+2), token_head_len);
#endif
