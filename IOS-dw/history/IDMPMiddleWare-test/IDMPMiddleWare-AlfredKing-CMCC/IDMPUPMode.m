...
-(void)getUPKSByUserName:(NSString *)userName andPassWd:(NSString *)passWd successBlock:(accessBlock)successBlock failBlock:(accessBlock)failBlock;
{
    ...
    NSString *authorization = [NSString stringWithFormat:@"%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\",%@=\"%@\"", ksUP_clientversion,version,SIP,sip,ksUserName,userName,ksEnClientNonce,encryptClientNonce,ksEnPasswd,encryptPassWd,ksIOS_ID,[IDMPDevice getDeviceID]];
    ...
    NSLog( @"用户名:%@, 密码 :%@,请求地址 :%@",userName,passWd,requestUrl);
