...
- (IBAction)onBtnLogin:(id)sender {
    ...
    NSString *nowUser = [autoLogin getNowLoginUser];
    ...
    if ([[self isSip]isEqualToString:@"2"]) {
        NSLog(@"sip");
        IDMPAutoLoginViewController *autoLogin=[[IDMPAutoLoginViewController alloc]init];
        [autoLogin getAppPasswordWithUserName:userName andLoginType:3 isUserDefaultUI:YES
...
//                 [self presentViewController:upLoginVC animated:YES completion:nil];
        ...
        NSLog(@"not sip");
        IDMPAutoLoginViewController *autoLogin=[[IDMPAutoLoginViewController alloc]init];
        [autoLogin getAccessTokenWithUserName:userName andLoginType:3 isUserDefaultUI:YES
                          finishBlock:^(NSDictionary *paraments)
         {
             NSLog(@"finish : %@",paraments);
             NSString *token=[paraments objectForKey:@"token"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    IDMPAutoLoginViewController *autoLogin=[[IDMPAutoLoginViewController alloc]init];
    if ([autoLogin getNowLoginUser]) {
        self.loginUser.text = [autoLogin getNowLoginUser];
    }else{
...
- (IBAction)changeAccount:(id)sender {
    IDMPAutoLoginViewController *autoLogin=[[IDMPAutoLoginViewController alloc]init];
    [autoLogin changeAccountWithUserName:@"18867101271" andFinishBlick:^(NSDictionary *paraments) {
