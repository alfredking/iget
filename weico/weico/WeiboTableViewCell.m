...
@interface WeiboTableViewCell()<WeiboLabelDelegate>{
...
- (instancetype) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        //装饰属性、可能会有小箭头，自定义附件按钮的其他样式，必需使用UITableView 的 accessoryView 属性https://www.jianshu.com/p/83c6af8690ee
...
- (void) initSubView{
    ...
    _userName.font = WBStatusCellDetailFont;
    ...
    //微博视频
    _weiboVideoView = [[WeiboVideoView alloc] init];
    [self.containView addSubview:_weiboVideoView];
...
- (void)setWeiboMsg:(WeiboMsg *)weiboMsg{
    ...
    weiboMsg.video_url = @"https://f.video.weibocdn.com/004wXFpZgx07ERnKypKU01041200jcxQ0E010.mp4?label=mp4_hd&template=720x1280.24.0&trans_finger=e8a052b5584408bcb51fcafb9761cd6d&Expires=1595158545&ssig=iahyEelw1%2F&KID=unistore,video";
...
//    NSLog(@"msg usr is %@",weiboMsg.user);
//    NSLog(@"msg usr is %@",weiboMsg.user.name);
    ...
    NSLog(@"weiboMsg.user.name is %@",weiboMsg.user.name);
    NSLog(@"username size height is %f,username size width is %f",userNameSize.height,userNameSize.width);
    //用户名计算的size不够
    CGFloat userNameSizeX = userNameSize.width+50;
    ...
    if (weiboMsg.retweeted_status) {
        ...
        if (weiboMsg.retweeted_status.thumbnail_pic) {
            ...
            //有视频
            if (weiboMsg.video_url)
            {
                _weiboVideoView.hidden = NO;
                
                CGFloat videoX = 10;
                CGFloat videoY = CGRectGetMaxY(_wbForwardText.frame) + WBStatusCellControlSpacing;
                CGFloat videoWidth = IPHONE_SCREEN_WIDTH - WBStatusCellControlSpacing*2;
                CGRect videoRect = CGRectMake(videoX, videoY, videoWidth, 200);
                [_weiboVideoView initWithFrame:videoRect];
                [_weiboVideoView layoutWithVideoCoverUrl:@"icon.bundle/videoCover@3x.png" videoUrl:weiboMsg.video_url];
                imageHeight =CGRectGetMaxY(_weiboVideoView.frame) ;            }
            else
            {
                //没有视频
    ...
    //无转发
       ...
       if (weiboMsg.thumbnail_pic) {
           ...
           if (weiboMsg.video_url)
           {
               ...
               CGFloat videoX = 10;
               CGFloat videoY = CGRectGetMaxY(_wbDetail.frame) + WBStatusCellControlSpacing;
               CGFloat videoWidth = IPHONE_SCREEN_WIDTH - WBStatusCellControlSpacing*2;
               CGRect videoRect = CGRectMake(videoX, videoY, videoWidth, 200);
               NSLog(@" video rect is %@", NSStringFromCGRect(videoRect));
               _weiboVideoView.frame = videoRect;
               _weiboVideoView.backgroundColor = [UIColor greenColor];
               [_weiboVideoView layoutWithVideoCoverUrl:@"icon.bundle/videoCover@3x.png" videoUrl:weiboMsg.video_url];
               imageHeight =CGRectGetMaxY(_weiboVideoView.frame) ;            }
           else
           {
    ...
    NSLog(@"weiboMsg.comments_count.integerValue is %ld",(long)weiboMsg.comments_count.integerValue);
    ...
    //不想要评论按钮就这样设置
//    containViewRect = CGRectMake(5, 5, IPHONE_SCREEN_WIDTH - 10, imageHeight + 5);
    ...
    [_praiseBtn addTarget:self action:@selector(like) forControlEvents:UIControlEventTouchUpInside];
...
- (void)comment {
    ...
    NSLog(@"comment");
//    [self bounceTargetView:_commentBtn];
...
- (void)forward {
    NSLog(@"forward");
//    [self bounceTargetView:_forwardBtn];
...
- (void)like {
//    [self bounceTargetView:_praiseBtn];
    NSLog(@"like");
}
