...
- (void)viewDidLoad {
    ...
    //使用场景：当父视图更改了tintColor为red，那么它所有的一级子视图tintColor全部为red，除非你主动设置过子视图的tintColor。它再往下的子视图的tintColor则遵循直接父视图的tintColor
  ...
  //ClipsToBounds</pre>属性设置成YES的时，剪裁超出父视图范围的子视图部分。当设置成NO的时候，不剪裁超出父视图范围的子视图。
